...
public final class RMPenilaianAwalKeperawatanRalanGeriatri extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        TglAsuhan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "10-06-2023 07:29:18" }));
            ...
            if(Sequel.menyimpantf("penilaian_awal_keperawatan_ralan_geriatri","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",80,new String[]{
                    ...
                    for (i = 0; i < tbRencanaEtiologi.getRowCount(); i++) {
                        if(tbRencanaEtiologi.getValueAt(i,0).toString().equals("true")){
                            Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_etiologi_geriatri","?,?",2,new String[]{TNoRw.getText(),tbRencanaEtiologi.getValueAt(i,1).toString()});
                        }
                    }
                    for (i = 0; i < tbRencanaLuaran.getRowCount(); i++) {
                        if(tbRencanaLuaran.getValueAt(i,0).toString().equals("true")){
                            Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_luaran_geriatri","?,?",2,new String[]{TNoRw.getText(),tbRencanaLuaran.getValueAt(i,1).toString()});
                        }
                    }
                    emptTeks();
            }
    ...
    */
        ...
        if(tbObat.getSelectedRow()!= -1){
            ...
            try {
                ...
                ps=koneksi.prepareStatement(
                        ...
                        "inner join penilaian_awal_keperawatan_ralan_etiologi_geriatri on penilaian_awal_keperawatan_ralan_etiologi_geriatri.kode_etiologi=master_etiologi_keperawatan.kode_etiologi "+
                        "where penilaian_awal_keperawatan_ralan_etiologi_geriatri.no_rawat=? order by penilaian_awal_keperawatan_ralan_etiologi_geriatri.kode_etiologi");
                try {
                    ps.setString(1,tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                    rs=ps.executeQuery();
                    while(rs.next()){
            ...
            try {
                ...
                ps=koneksi.prepareStatement(
                        ...
                        "inner join penilaian_awal_keperawatan_ralan_luaran_geriatri on penilaian_awal_keperawatan_ralan_luaran_geriatri.kode_luaran=master_luaran_keperawatan.kode_luaran "+
                        "where penilaian_awal_keperawatan_ralan_luaran_geriatri.no_rawat=? order by penilaian_awal_keperawatan_ralan_luaran_geriatri.kode_luaran");
                try {
                    ps.setString(1,tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                    rs=ps.executeQuery();
                    while(rs.next()){
    ...
    private void tampilEtiologi() {
        try{
            file=new File("./cache/rencanaetiologigeriatri.iyem");
            file.createNewFile();
            fileWriter = new FileWriter(file);
            iyem="";
            ...
            fileWriter.write("{\"rencanaetiologigeriatri\":["+iyem.substring(0,iyem.length()-1)+"]}");
            fileWriter.flush();
            fileWriter.close();
            iyem=null;
        }catch(Exception e){
    ...
    private void tampilEtiologi2() {
        try{
            ...
            myObj = new FileReader("./cache/rencanaetiologigeriatri.iyem");
            root = mapper.readTree(myObj);
            response = root.path("rencanaetiologigeriatri");
            if(response.isArray()){
                for(i=0;i<tbMasalahKeperawatan.getRowCount();i++){
                    if(tbMasalahKeperawatan.getValueAt(i,0).toString().equals("true")){
                        for(JsonNode list:response){
     ...
     private void tampilLuaran() {
        try{
            file=new File("./cache/rencanaluarangeriatri.iyem");
            file.createNewFile();
            fileWriter = new FileWriter(file);
            iyem="";
            ...
            fileWriter.write("{\"rencanaluarangeriatri\":["+iyem.substring(0,iyem.length()-1)+"]}");
            fileWriter.flush();
            fileWriter.close();
            iyem=null;
        }catch(Exception e){
    ...
    private void tampilLuaran2() {
        try{
            ...
            myObj = new FileReader("./cache/rencanaluarangeriatri.iyem");
            root = mapper.readTree(myObj);
            response = root.path("rencanaluarangeriatri");
            if(response.isArray()){
                for(i=0;i<tbMasalahKeperawatan.getRowCount();i++){
                    if(tbMasalahKeperawatan.getValueAt(i,0).toString().equals("true")){
                        for(JsonNode list:response){
    ...
    private void hapus() {
        if(Sequel.queryu2tf("delete from penilaian_awal_keperawatan_ralan_geriatri where no_rawat=?",1,new String[]{
            ...
            Sequel.meghapus("penilaian_awal_keperawatan_ralan_etiologi_geriatri","no_rawat",tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
            Sequel.meghapus("penilaian_awal_keperawatan_ralan_luaran_geriatri","no_rawat",tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
            Valid.tabelKosong(tabModeDetailMasalah);
            Valid.tabelKosong(tabModeDetailRencana);
            Valid.tabelKosong(tabModeDetailEtiologi);
            Valid.tabelKosong(tabModeDetailLuaran);
