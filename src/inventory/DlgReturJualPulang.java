...
public class DlgReturJualPulang extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgReturJualPulang(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void BtnTambahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnTambahActionPerformed
        if(nmbar.getText().trim().equals("")){
            ...
            if(formvalid.equals("No")){
                if(aktifkanbatch.equals("yes")){
                    if(Double.parseDouble(Jmlretur.getText())<=Sequel.cariIsiAngka("select sum(jml) from detail_pemberian_obat_pulang where no_rawat='"+norawat+"' and kode_brng='"+Kdbar.getText()+"' and no_batch='"+NoBatch.getText()+"' and no_faktur='"+NoFaktur.getText()+"'")){
                    ...
                    if(Double.parseDouble(Jmlretur.getText())<=Sequel.cariIsiAngka("select sum(jml) from detail_pemberian_obat_pulang where no_rawat='"+norawat+"' and kode_brng='"+Kdbar.getText()+"' and no_batch='' and no_faktur=''")){
                        if(Sequel.menyimpantf("tampreturjual","'"+NoNota.getText()+"','"+Kdbar.getText()+"','"+nmbar.getText()+"','0','0','"+Jmlretur.getText()+"','"+Hargaretur.getText()+"','"+Satuanbar.getText()+"','"+Subtotal.getText()+"','"+NoBatch.getText()+"','"+akses.getkode()+"','"+NoFaktur.getText()+"'","Kode Barang, No.Batch, No.Faktur")==true){
                            emptTeks();            
                            tampil();
                        }
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgReturJualPulang dialog = new DlgReturJualPulang(new javax.swing.JFrame(), true);
                ...
                while(rs.next()){
                    if(Sequel.menyimpantf2("detreturjual","?,?,?,?,?,?,?,?,?,?,?","data barang sama",11,new String[]{
                        ...
                        if(aktifkanbatch.equals("yes")){
                            ...
                            Sequel.mengedit("detail_pemberian_obat_pulang","no_rawat=?","jml=jml-?",2,new String[]{
...
//                            Sequel.mengedit("detail_pemberian_obat_pulang","no_rawat=?","jml=jml-?",2,new String[]{
//                              rw.getText(),rs.getString(6)});
                            ps = koneksi.prepareStatement("update detail_pemberian_obat_pulang set jml=jml-? where no_rawat like ? and kode_brng like ?");
