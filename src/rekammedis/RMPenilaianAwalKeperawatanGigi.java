            ...
            if(Sequel.menyimpantf("penilaian_awal_keperawatan_gigi","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",57,new String[]{
                    ...
                    for (i = 0; i < tbRencanaEtiologi.getRowCount(); i++) {
                        if(tbRencanaEtiologi.getValueAt(i,0).toString().equals("true")){
                            Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_etiologi_gigi","?,?",2,new String[]{TNoRw.getText(),tbRencanaEtiologi.getValueAt(i,1).toString()});
                        }
                    }
                    for (i = 0; i < tbRencanaLuaran.getRowCount(); i++) {
                        if(tbRencanaLuaran.getValueAt(i,0).toString().equals("true")){
                            Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_luaran_gigi","?,?",2,new String[]{TNoRw.getText(),tbRencanaLuaran.getValueAt(i,1).toString()});
                        }
                    }
                    emptTeks();
            }
    ...
    */
        ...
        if(tbObat.getSelectedRow()!= -1){
            ...
            try {
                ...
                ps=koneksi.prepareStatement(
                        ...
                        "where penilaian_awal_keperawatan_ralan_etiologi_gigi.no_rawat=? order by penilaian_awal_keperawatan_ralan_etiologi_gigi.kode_etiologi");
                try {
                    ps.setString(1,tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                    rs=ps.executeQuery();
                    while(rs.next()){
            ...
            try {
                ...
                ps=koneksi.prepareStatement(
                        ...
                        "where penilaian_awal_keperawatan_ralan_luaran_gigi.no_rawat=? order by penilaian_awal_keperawatan_ralan_luaran_gigi.kode_luaran");
                try {
                    ps.setString(1,tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                    rs=ps.executeQuery();
                    while(rs.next()){
    ...
    private void ganti() {
        if(Sequel.mengedittf("penilaian_awal_keperawatan_gigi","no_rawat=?","no_rawat=?,tanggal=?,informasi=?,td=?,nadi=?,rr=?,suhu=?,bb=?,tb=?,bmi=?,keluhan_utama=?,riwayat_penyakit=?,ket_riwayat_penyakit=?,alergi=?,riwayat_perawatan_gigi=?,ket_riwayat_perawatan_gigi=?,kebiasaan_sikat_gigi=?,kebiasaan_lain=?,ket_kebiasaan_lain=?,obat_yang_diminum_saatini=?,alat_bantu=?,ket_alat_bantu=?,prothesa=?,ket_pro=?,status_psiko=?,ket_psiko=?,hub_keluarga=?,tinggal_dengan=?,ket_tinggal=?,ekonomi=?,budaya=?,ket_budaya=?,edukasi=?,ket_edukasi=?,berjalan_a=?,berjalan_b=?,berjalan_c=?,hasil=?,lapor=?,ket_lapor=?,nyeri=?,lokasi=?,skala_nyeri=?,durasi=?,frekuensi=?,nyeri_hilang=?,ket_nyeri=?,pada_dokter=?,ket_dokter=?,kebersihan_mulut=?,mukosa_mulut=?,karies=?,karang_gigi=?,gingiva=?,palatum=?,rencana=?,nip=?",58,new String[]{
                ...
                Sequel.meghapus("penilaian_awal_keperawatan_ralan_etiologi_gigi","no_rawat",tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                for (i = 0; i < tbRencanaEtiologi.getRowCount(); i++) {
                    if(tbRencanaEtiologi.getValueAt(i,0).toString().equals("true")){
                        Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_etiologi_gigi","?,?",2,new String[]{TNoRw.getText(),tbRencanaEtiologi.getValueAt(i,1).toString()});
                    }
                }
                Sequel.meghapus("penilaian_awal_keperawatan_ralan_luaran_gigi","no_rawat",tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                for (i = 0; i < tbRencanaLuaran.getRowCount(); i++) {
                    if(tbRencanaLuaran.getValueAt(i,0).toString().equals("true")){
                        Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_luaran_gigi","?,?",2,new String[]{TNoRw.getText(),tbRencanaLuaran.getValueAt(i,1).toString()});
                    }
