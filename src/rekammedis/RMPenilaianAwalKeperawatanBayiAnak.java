...
public final class RMPenilaianAwalKeperawatanBayiAnak extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        TglAsuhan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "20-09-2023 09:21:04" }));
        ...
        tbMasalahKeperawatan.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mousePressed(java.awt.event.MouseEvent evt) {
                tbMasalahKeperawatanMousePressed(evt);
            }
        });
            ...
            if(Sequel.menyimpantf("penilaian_awal_keperawatan_ralan_bayi","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",85,new String[]{
                    ...
                    for (i = 0; i < tbRencanaEtiologi.getRowCount(); i++) {
                        if(tbRencanaEtiologi.getValueAt(i,0).toString().equals("true")){
                            Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_etiologi_anak","?,?",2,new String[]{TNoRw.getText(),tbRencanaEtiologi.getValueAt(i,1).toString()});
                        }
                    }
                    for (i = 0; i < tbRencanaLuaran.getRowCount(); i++) {
                        if(tbRencanaLuaran.getValueAt(i,0).toString().equals("true")){
                            Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_luaran_anak","?,?",2,new String[]{TNoRw.getText(),tbRencanaLuaran.getValueAt(i,1).toString()});
                        }
                    }
                    emptTeks();
            }
    ...
    private void tbMasalahKeperawatanMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbMasalahKeperawatanMousePressed
       
    }//GEN-LAST:event_tbMasalahKeperawatanMousePressed
    ...
    */
        ...
        if(tbObat.getSelectedRow()!= -1){
            ...
            try {
                ...
                ps=koneksi.prepareStatement(
                        ...
                        "where penilaian_awal_keperawatan_ralan_etiologi_anak.no_rawat=? order by penilaian_awal_keperawatan_ralan_etiologi_anak.kode_etiologi");
                try {
                    ps.setString(1,tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                    rs=ps.executeQuery();
                    while(rs.next()){
            ...
            try {
                ...
                ps=koneksi.prepareStatement(
                        ...
                        "where penilaian_awal_keperawatan_ralan_luaran_anak.no_rawat=? order by penilaian_awal_keperawatan_ralan_luaran_anak.kode_luaran");
                try {
                    ps.setString(1,tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                    rs=ps.executeQuery();
                    while(rs.next()){
    ...
    private void ganti(){
        if(Sequel.mengedittf("penilaian_awal_keperawatan_ralan_bayi","no_rawat=?","no_rawat=?,tanggal=?,informasi=?,td=?,nadi=?,rr=?,suhu=?,gcs=?,bb=?,tb=?,lp=?,lk=?,ld=?,keluhan_utama=?,rpd=?,rpk=?,rpo=?,alergi=?,anakke=?,darisaudara=?,caralahir=?,ket_caralahir=?,umurkelahiran=?,kelainanbawaan=?,ket_kelainan_bawaan=?,usiatengkurap=?,usiaduduk=?,usiaberdiri=?,usiagigipertama=?,usiaberjalan=?,usiabicara=?,usiamembaca=?,usiamenulis=?,gangguanemosi=?,alat_bantu=?,ket_bantu=?,prothesa=?,ket_pro=?,adl=?,status_psiko=?,ket_psiko=?,hub_keluarga=?,pengasuh=?,ket_pengasuh=?,ekonomi=?,budaya=?,ket_budaya=?,edukasi=?,ket_edukasi=?,berjalan_a=?,berjalan_b=?,berjalan_c=?,hasil=?,lapor=?,ket_lapor=?,sg1=?,nilai1=?,sg2=?,nilai2=?,sg3=?,nilai3=?,sg4=?,nilai4=?,total_hasil=?,wajah=?,nilaiwajah=?,kaki=?,nilaikaki=?,aktifitas=?,nilaiaktifitas=?,menangis=?,nilaimenangis=?,bersuara=?,nilaibersuara=?,hasilnyeri=?,nyeri=?,lokasi=?,durasi=?,frekuensi=?,nyeri_hilang=?,ket_nyeri=?,pada_dokter=?,ket_dokter=?,rencana=?,nip=?",86,new String[]{
                ...
                Sequel.meghapus("penilaian_awal_keperawatan_ralan_etiologi_anak","no_rawat",tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                for (i = 0; i < tbRencanaEtiologi.getRowCount(); i++) {
                    if(tbRencanaEtiologi.getValueAt(i,0).toString().equals("true")){
                        Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_etiologi_anak","?,?",2,new String[]{TNoRw.getText(),tbRencanaEtiologi.getValueAt(i,1).toString()});
                    }
                }
                Sequel.meghapus("penilaian_awal_keperawatan_ralan_luaran_anak","no_rawat",tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                for (i = 0; i < tbRencanaLuaran.getRowCount(); i++) {
                    if(tbRencanaLuaran.getValueAt(i,0).toString().equals("true")){
                        Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_luaran_anak","?,?",2,new String[]{TNoRw.getText(),tbRencanaLuaran.getValueAt(i,1).toString()});
                    }
