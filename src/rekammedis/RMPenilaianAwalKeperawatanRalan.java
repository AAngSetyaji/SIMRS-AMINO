...
public final class RMPenilaianAwalKeperawatanRalan extends javax.swing.JDialog {
            ...
            if(Sequel.menyimpantf("penilaian_awal_keperawatan_ralan","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",57,new String[]{
                    ...
                    for (i = 0; i < tbRencanaEtiologi.getRowCount(); i++) {
                        if(tbRencanaEtiologi.getValueAt(i,0).toString().equals("true")){
                            Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_etiologi","?,?",2,new String[]{TNoRw.getText(),tbRencanaEtiologi.getValueAt(i,1).toString()});
                        }
                    }
                    for (i = 0; i < tbRencanaLuaran.getRowCount(); i++) {
                        if(tbRencanaLuaran.getValueAt(i,0).toString().equals("true")){
                            Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_luaran","?,?",2,new String[]{TNoRw.getText(),tbRencanaLuaran.getValueAt(i,1).toString()});
                        }
                    }
                    emptTeks();
            }
    ...
    */
        ...
        if(tbObat.getSelectedRow()!= -1){
            ...
            try {
                ...
                ps=koneksi.prepareStatement(
                        ...
                        "where penilaian_awal_keperawatan_ralan_etiologi.no_rawat=? order by penilaian_awal_keperawatan_ralan_etiologi.kode_etiologi");
                try {
                    ps.setString(1,tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                    rs=ps.executeQuery();
                    while(rs.next()){
            ...
            try {
                ...
                ps=koneksi.prepareStatement(
                        ...
                        "where penilaian_awal_keperawatan_ralan_luaran.no_rawat=? order by penilaian_awal_keperawatan_ralan_luaran.kode_luaran");
                try {
                    ps.setString(1,tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                    rs=ps.executeQuery();
                    while(rs.next()){
    ...
    private void tampilEtiologi() {
        try{
            file=new File("./cache/rencanaetiologiralan.iyem");
            file.createNewFile();
            fileWriter = new FileWriter(file);
            iyem="";
            ...
            fileWriter.write("{\"rencanaetiologiralan\":["+iyem.substring(0,iyem.length()-1)+"]}");
            fileWriter.flush();
            fileWriter.close();
            iyem=null;
        }catch(Exception e){
    ...
    private void tampilEtiologi2() {
        try{
            ...
            myObj = new FileReader("./cache/rencanaetiologiralan.iyem");
            root = mapper.readTree(myObj);
            response = root.path("rencanaetiologiralan");
            if(response.isArray()){
                for(i=0;i<tbMasalahKeperawatan.getRowCount();i++){
                    if(tbMasalahKeperawatan.getValueAt(i,0).toString().equals("true")){
                        for(JsonNode list:response){
     ...
     private void tampilLuaran() {
        try{
            file=new File("./cache/rencanaluaranralan.iyem");
            file.createNewFile();
            fileWriter = new FileWriter(file);
            iyem="";
            ...
            fileWriter.write("{\"rencanaluaranralan\":["+iyem.substring(0,iyem.length()-1)+"]}");
            fileWriter.flush();
            fileWriter.close();
            iyem=null;
        }catch(Exception e){
    ...
    private void tampilLuaran2() {
        try{
            ...
            myObj = new FileReader("./cache/rencanaluaranralan.iyem");
            root = mapper.readTree(myObj);
            response = root.path("rencanaluaranralan");
            if(response.isArray()){
                for(i=0;i<tbMasalahKeperawatan.getRowCount();i++){
                    if(tbMasalahKeperawatan.getValueAt(i,0).toString().equals("true")){
                        for(JsonNode list:response){
    ...
    private void ganti() {
        if(Sequel.mengedittf("penilaian_awal_keperawatan_ralan","no_rawat=?","no_rawat=?,tanggal=?,informasi=?,td=?,nadi=?,rr=?,suhu=?,gcs=?,bb=?,tb=?,bmi=?,keluhan_utama=?,rpd=?,rpk=?,rpo=?,alergi=?,alat_bantu=?,ket_bantu=?,prothesa=?,ket_pro=?,adl=?,status_psiko=?,ket_psiko=?,hub_keluarga=?,tinggal_dengan=?,ket_tinggal=?,ekonomi=?,budaya=?,ket_budaya=?,edukasi=?,ket_edukasi=?,berjalan_a=?,berjalan_b=?,berjalan_c=?,hasil=?,lapor=?,ket_lapor=?,sg1=?,nilai1=?,sg2=?,nilai2=?,total_hasil=?,nyeri=?,provokes=?,ket_provokes=?,quality=?,ket_quality=?,lokasi=?,menyebar=?,skala_nyeri=?,durasi=?,nyeri_hilang=?,ket_nyeri=?,pada_dokter=?,ket_dokter=?,rencana=?,nip=?",58,new String[]{
                ...
                Sequel.meghapus("penilaian_awal_keperawatan_ralan_etiologi","no_rawat",tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                for (i = 0; i < tbRencanaEtiologi.getRowCount(); i++) {
                    if(tbRencanaEtiologi.getValueAt(i,0).toString().equals("true")){
                        Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_etiologi","?,?",2,new String[]{TNoRw.getText(),tbRencanaEtiologi.getValueAt(i,1).toString()});
                    }
                }
                Sequel.meghapus("penilaian_awal_keperawatan_ralan_luaran","no_rawat",tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                for (i = 0; i < tbRencanaLuaran.getRowCount(); i++) {
                    if(tbRencanaLuaran.getValueAt(i,0).toString().equals("true")){
                        Sequel.menyimpan2("penilaian_awal_keperawatan_ralan_luaran","?,?",2,new String[]{TNoRw.getText(),tbRencanaLuaran.getValueAt(i,1).toString()});
                    }
